...
: G4VUserDetectorConstruction(), 
  fMessenger(nullptr), cellEcalBarLogical(nofEmBarEta),
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(k<kNofEmECCells)
	{
		...
		if(d1>rmin_EC && d1<rmax_EC)
		{
			...
			auto cellEcalECSolid_r = new G4Trap("cellEcalECSolid_r", 0.5*len*mm, d_r->getTheta()*rad,
		            d_r->getPhi()*rad, 0.5*y1*mm, 0.5*y1*mm, 0.5*y1*mm, 0*deg, 0.5*y2*mm,
	             	0.5*y2*mm, 0.5*y2*mm, 0*deg);
			cellEcalECLogical_r[k] = new G4LogicalVolume(cellEcalECSolid_r, ecal_mat, "CellEcalECLogical_r");
			G4ThreeVector* d_l = new G4ThreeVector(xpos_EC, ypos_EC, -(3205+110+1300));
			auto cellEcalECSolid_l = new G4Trap("cellEcalECSolid_l", 0.5*len*mm, d_l->getTheta()*rad,
		            d_l->getPhi()*rad, 0.5*y2*mm, 0.5*y2*mm, 0.5*y2*mm, 0*deg, 0.5*y1*mm,
	             	0.5*y1*mm, 0.5*y1*mm, 0*deg);
			cellEcalECLogical_l[k] = new G4LogicalVolume(cellEcalECSolid_l, ecal_mat, "CellEcalECLogical_l");
				if(ECalR)
